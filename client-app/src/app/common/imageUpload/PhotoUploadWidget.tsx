...
export default observer(function PhotoUploadWidget({loading,uploadPhoto}: Props){
    ...
    const[cropper,seCropper]=useState<Cropper>();
    function onCrop() {
        if (cropper) {
            ...
            <Grid.Column width={4}>
                <Header sub color='teal' content='Step 2 -Resize image' />
                {
                    files && files.length >0 && (                       
                        <PhotoWidgetCropper setCropper={seCropper} imagePreview={files[0].preview} />
                    )
                }
            </Grid.Column>
            <Grid.Column width={1}/>
            <Grid.Column width={4}>
                <Header sub color='teal' content='Step 3 -Previe & upload' />
